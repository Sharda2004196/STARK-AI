...
try:
    from cloakbrowser import async_launch as cloak_launch, async_launch_persistent_context as cloak_launch_persistent
...
class _BrowserSession:
    ...
    def __init__(self, browser_name: str, incognito: bool = False):
    ...
    async def _launch(self):
        ...
        Chromium tabanlılar için CloakBrowser (stealth) kullanılır.
        """
        ...
        # Standard Playwright for non-chromium
        if engine_name in ("firefox", "webkit"):
            if engine_name == "firefox":
                ...
                profile = str(Path.home() / ".jarvis_profiles" / "safari")
            ...
            try:
                if self.incognito:
                    self._browser = await engine_obj.launch(**kwargs)
                    self._context = await self._browser.new_context()
                else:
                    self._context = await engine_obj.launch_persistent_context(profile, **kwargs)
            except Exception as e:
                if self.incognito:
                    raise RuntimeError(f"Could not launch {engine_name} in incognito: {e}")
                print(f"[Browser] {engine_name} real profile failed ({e}), using fallback")
                fallback = str(Path.home() / ".jarvis_profiles" / f"{engine_name}_fallback")
                Path(fallback).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(fallback, **kwargs)

            await asyncio.sleep(0.5)  
            self._page = await self._context.new_page()
            print(f"[Browser] ✅ {engine_name.capitalize()} launched")
            return

        # Chromium based -> Use CloakBrowser
        ...
        if CLOAK_AVAILABLE:
            kwargs["humanize"] = True  # Enable stealthy interactions
            # Note: CloakBrowser manages its own binary, so we usually don't pass exe
            # unless we want to override it. We'll try without first to use the patched one.
            launch_func = cloak_launch_persistent
        else:
            # Fallback to standard playwright if cloak is missing
            launch_func = engine_obj.launch_persistent_context
            kwargs["slow_mo"] = 50
            if exe:
                kwargs["executable_path"] = exe
            elif channel:
        ...
        label = f"{self.browser_name} (Cloak)" if CLOAK_AVAILABLE else f"{self.browser_name} (Stock)"

        try:
            self._context = await launch_func(profile, **kwargs)
            await asyncio.sleep(0.5) 
            self._page = await self._context.new_page()
            ...
            try:
                self._context = await launch_func(jarvis_profile, **kwargs)
                await asyncio.sleep(0.5)
                self._page = await self._context.new_page()
...
class _SessionRegistry:
    ...
    def _get_or_create(self, browser_name: str, incognito: bool = False) -> _BrowserSession:
        key = f"{browser_name}_incognito" if incognito else browser_name
        with self._lock:
            if key not in self._sessions:
                sess = _BrowserSession(browser_name, incognito)
                sess.start()
                self._sessions[key] = sess
                print(f"[Registry] New session: {key}")
            return self._sessions[key]

    def get(self, browser_name: str | None = None, incognito: bool = False) -> _BrowserSession:
        if incognito and not browser_name:
            browser_name = "edge"
        
        if not browser_name:
            # If we fall back to active_browser, strip _incognito from it to get the base browser name
            active_base = self._active_browser.replace("_incognito", "") if self._active_browser else ""
            browser_name = active_base or _detect_default_browser()
            
        browser_name = _ALIASES.get(browser_name.lower().strip(), browser_name.lower().strip())
        sess = self._get_or_create(browser_name, incognito)
        self._active_browser = f"{browser_name}_incognito" if incognito else browser_name
        return sess

    def switch(self, browser_name: str, incognito: bool = False) -> str:
        browser_name = _ALIASES.get(browser_name.lower().strip(), browser_name.lower().strip())
        self._get_or_create(browser_name, incognito)
        key = f"{browser_name}_incognito" if incognito else browser_name
        self._active_browser = key
        return f"Active browser → {key}"
...
def browser_control(
    ...
    if action == "switch":
        ...
        result = _registry.switch(target, incognito) if target else "Please specify a browser."
        _log(player, result)
        return result
    ...
    try:
        sess = _registry.get(browser, incognito)
    except Exception as e:

...
def close_app(name: str = None):
    if _OS == "Darwin":
        if name:
            # Prevent closing Jarvis itself on Mac
            name_lower = name.lower()
            if any(k in name_lower for k in ("jarvis", "stark", "main.py")):
                print(f"[Settings] 🛡️ Blocked attempt to close Jarvis: {name}")
                return
        ...
        if name:
            # Prevent closing Jarvis itself
            name_lower = name.lower()
            if any(k in name_lower for k in ("jarvis", "stark", "main.py", "python")):
                # Check if it's really us or just another python script
                if name_lower in ("jarvis", "stark"):
                    print(f"[Settings] 🛡️ Blocked attempt to close Jarvis: {name}")
                    return

            # Try psutil for a cleaner match on Windows/Linux
            if _PSUTIL:
                try:
                    target = name.lower()
                    # Common aliases
                    aliases = {
                        ...
                        "notepad": "notepad.exe", "explorer": "explorer.exe", "vscode": "code.exe",
                    ...
                    for proc in psutil.process_iter(['name', 'exe', 'cmdline']):
                        try:
                            # Use name and exe path for matching
                            proc_name = (proc.info['name'] or "").lower()
                            proc_exe  = (proc.info['exe'] or "").lower()
                            
                            if target in proc_name or target in proc_exe:
                                # Safety: Don't kill ourself
                                if "python" in proc_name:
                                    cmdline = " ".join(proc.info.get('cmdline') or []).lower()
                                    if any(k in cmdline for k in ("main.py", "jarvis", "stark")):
                                        continue
                            ...
                            continue
                    if found:
                        print(f"[Settings] Closed {name} via psutil.")
                        return
                except Exception as e:
                    print(f"[Settings] psutil termination failed: {e}")

            # Fallback to pywinauto if psutil didn't work or isn't available
            try:
                from pywinauto import Application
                app = Application(backend="win32")
                app.connect(title_re=f".*{re.escape(name)}.*", timeout=3)
                
                title = app.top_window().window_text()
                if any(k in title.upper() for k in ("JARVIS", "STARK")):
                    print(f"[Settings] 🛡️ Safety: Connected to Jarvis window '{title}', aborting close.")
                    return

                app.top_window().close()
                return
            except Exception as e:
                print(f"[Settings] pywinauto close failed for '{name}': {e}")
                # DO NOT fallback to Alt+F4 if a name was specified but failed.
                # That's how Jarvis accidentally closes himself.
        else:
            # No name provided, close the active window
...
def toggle_bluetooth(target_state: str = None):
    """Toggle Bluetooth or set to specific state ('on'/'off')."""
    if _OS == "Windows":
        try:
            # Using modern Windows.Devices.Radios API via PowerShell
            ps_cmd = (
                "$radios = [Windows.Devices.Radios.Radio, Windows.Devices.Radios, ContentType=WindowsRuntime]::GetRadiosAsync(); "
                "$results = [WindowsRuntimeSystemExtensions]::GetAwaiter($radios).GetResult(); "
                "$bluetooth = $results | Where-Object { $_.Kind -eq 'Bluetooth' }; "
            )
            
            if target_state:
                new_state = "On" if target_state.lower() == "on" else "Off"
                ps_cmd += f"$bluetooth.SetStateAsync('{new_state}')"
            else:
                # Toggle
                ps_cmd += (
                    "if ($bluetooth.State -eq 'On') { $bluetooth.SetStateAsync('Off') } "
                    "else { $bluetooth.SetStateAsync('On') }"
                )
            
            subprocess.run(["powershell", "-Command", ps_cmd], capture_output=True, timeout=10)
        except Exception as e:
            print(f"[Settings] toggle_bluetooth Windows failed: {e}")
    elif _OS == "Darwin":
        try:
            # Requires blueutil: brew install blueutil
            state = "1" if target_state == "on" else "0" if target_state == "off" else "toggle"
            if state == "toggle":
                res = subprocess.run(["blueutil", "-p"], capture_output=True, text=True)
                state = "0" if "1" in res.stdout else "1"
            subprocess.run(["blueutil", "-p", state], capture_output=True)
        except Exception:
            pass
    else: # Linux
        try:
            state = "on" if target_state == "on" else "off" if target_state == "off" else "toggle"
            if state == "toggle":
                res = subprocess.run(["rfkill", "list", "bluetooth"], capture_output=True, text=True)
                state = "off" if "soft blocked: no" in res.stdout.lower() else "on"
            subprocess.run(["rfkill", "block" if state == "off" else "unblock", "bluetooth"], capture_output=True)
        except Exception:
            pass
...
ACTION_MAP: dict[str, callable] = {
    ...
    "wifi":                toggle_wifi,
    "wifi_on":             toggle_wifi,
    "wifi_off":            toggle_wifi,
    "enable_wifi":         toggle_wifi,
    "disable_wifi":        toggle_wifi,
    "turn_on_wifi":        toggle_wifi,
    "turn_off_wifi":       toggle_wifi,
    "toggle_bluetooth":    toggle_bluetooth,
    "bluetooth_on":        lambda: toggle_bluetooth("on"),
    "bluetooth_off":       lambda: toggle_bluetooth("off"),
    "enable_bluetooth":    lambda: toggle_bluetooth("on"),
    "disable_bluetooth":   lambda: toggle_bluetooth("off"),
    "turn_on_bluetooth":   lambda: toggle_bluetooth("on"),
    "turn_off_bluetooth":  lambda: toggle_bluetooth("off"),
    "action_enable_bluetooth":  lambda: toggle_bluetooth("on"),
    "action_disable_bluetooth": lambda: toggle_bluetooth("off"),
...
Rules:
- STRICTLY pick the single best matching action name FROM THE LIST BELOW. Do not invent names.
- CATEGORY - Bluetooth: bluetooth_on, bluetooth_off, toggle_bluetooth.
- CATEGORY - Wifi: turn_on_wifi, turn_off_wifi, toggle_wifi.
- If the user mentions "bluetooth", ONLY pick from the Bluetooth category.
- If the user mentions "wifi" or "internet", ONLY pick from the Wifi category.
- Use close_app if the user specifies a NAME of an application (e.g., "close Chrome", "exit Spotify").
- Use close_window if the user wants to close the CURRENT active window without naming it (e.g., "close this", "exit window").
...
- Return ONLY the JSON, no explanation, no markdown."""
    ...
    try:
        ...
        text = re.sub(r"```(?:json)?", "", resp).strip().rstrip("`").strip()
...
def computer_settings(
    ...
    if action in ("close_app", "close_window") and value:
        close_app(name=value)
        return f"Done: close_app {value}."

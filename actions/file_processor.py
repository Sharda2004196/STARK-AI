...
def _process_video(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    # Route advanced editing to video_editing module
    advanced_actions = ("animate", "add_text", "add_music", "merge", "beat_sync", 
                        "fade", "glow", "zoom", "fade_in_out", "video_edit_custom")
    if action in advanced_actions or action.startswith("video_edit"):
        try:
            from actions.video_editing import video_editing
            # Map action if needed
            v_action = action
            if action in ("fade", "glow", "zoom", "fade_in_out", "video_edit_custom"):
                v_action = "animate"
            
            v_params = {**params, "action": v_action, "video_path": str(path)}
            return video_editing(parameters=v_params, player=None, speak=speak)
        except Exception as e:
            return f"Advanced video editing failed: {e}"

    return f"Unknown video action: '{action}'. Try: info, trim, animate, add_text, add_music, extract_audio"

...
def _translate_to_goal_language(content: str, goal: str) -> str:
    ...
    try:
        ...
        translated = generate_content(model="gemini-3.1-flash-lite", contents=prompt).strip()
...
def _call_tool(tool: str, parameters: dict, speak: Callable | None) -> str:
    ...
    elif tool == "frontend_builder":
        from actions.frontend_builder import frontend_builder
        return frontend_builder(parameters=parameters, player=None, speak=speak) or "Done."

    elif tool == "prompt_optimizer":
        from actions.prompt_optimizer import prompt_optimizer
        return prompt_optimizer(parameters=parameters, player=None, speak=speak) or "Done."
    ...
    elif tool == "media_generation":
        from actions.media_generation import media_generation
        return media_generation(parameters=parameters, player=None) or "Done."
